...
// mismo ancho de contenido (padding horizontal 20 en los dos). Tocar una
// foto abre el feed vertical (PhotoFeedScreen, likes/comentarios) en esa
// posición.
const CONTENT_PADDING = 20;

export default function PhotoCarousel({ photos, churchId }) {
      ...
      >
        {photos.map((p, i) => (
          <TouchableOpacity
            key={p.id}
            activeOpacity={0.9}
            onPress={() => navigation.navigate("PhotoFeed", { churchId, initialIndex: i })}
          >
            <Image
